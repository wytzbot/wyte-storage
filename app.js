...
(function () {
  ...
  const TEST_NOTIF_LINES = {
    funny: "🔔 WYTE checking in! Yep, notifications are working. We can now annoy you responsibly 😂",
    simple: "Notifications are working.",
    professional: "This is a confirmation that WYTE Vault notifications are configured correctly."
  };
  document.getElementById("send-test-notif-btn").addEventListener("click", () => {
    if (!("Notification" in window) || Notification.permission !== "granted") {
      alert("Notifications aren't enabled yet, or OneSignal/Median isn't configured. Enable notifications above first.");
      return;
    }
    new Notification("WYTE Vault", { body: TEST_NOTIF_LINES[settings.notifStyle] || TEST_NOTIF_LINES.simple });
  });
  ...
  document.getElementById("open-pricing-btn").addEventListener("click", () => openSheet("sheet-pricing"));
